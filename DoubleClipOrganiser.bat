...
echo.
echo Checking for latest videos in:
echo   "%source%"
echo Target folder:
echo   "%target%"
echo.
        ...
        if exist "%%f" (
            for %%A in ("%%f") do (
                ...
                if !fileTime! gtr !newestTime! (
                    set "secondTime=!newestTime!"
                    set "secondFile=!newestFile!"
                    set "newestTime=!fileTime!"
                    set "newestFile=%%f"
                ) else if !fileTime! gtr !secondTime! (
                    set "secondTime=!fileTime!"
                    set "secondFile=%%f"
                )
            )
        )
...
if not defined secondFile (
    echo Only one video file found, nothing else to move.
    goto :eof
)

echo Moving second latest file:
echo   "!secondFile!"
echo To:
echo   "%target%"
move /Y "!secondFile!" "%target%" >nul
echo Move complete.
